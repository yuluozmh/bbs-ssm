...
@ApiIgnore
@RequestMapping("/api/rest/nansin/v3.0/user")
...
public class UserController {
    ...
    public ReturnT<?> updateUsername(String username, HttpSession session, Model model) {
        try {
            if (userService.getUserName(username) == null) {    // 用户名不存在-可以修改
                userService.updateUsername(username, (String) session.getAttribute("userid"));
    ...
    public ReturnT<?> updatePassword(String passOld, String passNew, HttpSession session) {
        ...
        try {
            if (userService.getIdPass(passOld, userid) != null) {    // 旧密码正确
                userService.updatePassword(passNew, userid);
    ...
    public ReturnT<?> updateEmail(String email, HttpSession session, Model model) {
        try {
            if (userService.getEmail(email) == null) {    // Email不存在-可以修改
                userService.updateEmail(email, (String) session.getAttribute("userid"));
    ...
    /**
     * 获取用户信息（名称、头像、粉丝数、文章总浏览、总点赞、总收藏）
     *
     * @param userid
     * @return
     */
    @GetMapping("/getAboutUser/{userid}")
    public ReturnT<?> getNewUser(@PathVariable String userid) {
        try {
            return new ReturnT<>(HttpStatus.OK, "获取用户数据成功", userService.getUserImplByUserid(userid));
        } catch (Exception e) {
            e.printStackTrace();
            return ReturnT.fail("获取用户数据失败");
        }
